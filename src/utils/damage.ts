...
const calculateDamage = (data: buffObj) => {
...
// 连锁伤害计算函数
const getChain = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 + (data.chainCount || 0) * (0.1 + (data.chainDamageAdd || 0) / 100)) // 连锁伤害
  return integer ? Math.floor(damage) : damage
}
// 倍率伤害计算函数
const getMultiplier = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = (damage * (data.multiplier || 100)) / 100 // 倍率
  return integer ? Math.floor(damage) : damage
}
// 易伤/脆弱 + 增伤伤害计算函数
const getEnemyWeakness = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 + (data.enemyWeakness || 0) / 100 + (data.increasedDamage || 0) / 100) // 易伤/脆弱 + 增伤
  return integer ? Math.floor(damage) : damage
}
// 属伤伤害计算函数
const getAttributeDamage = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = Math.floor(damage * (1 + (data.attributeDamage || 0) / 100)) // 属伤
  return integer ? Math.floor(damage) : damage
}
// 属伤伤害计算函数
const getCritical = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 + (data.critical || 0) / 100) // 爆伤
  return integer ? Math.floor(damage) : damage
}
// 弱点加伤计算函数
const getWeakPointDamageAdd = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 + (data.weakPointDamageAdd || 0) / 100) // 最后弱点加伤
  return integer ? Math.floor(damage) : damage
}
// 弱点加伤计算函数
const getAttackAdd = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 + (data.attackAdd || 0) / 100) // 攻击力/魔法力加成
  return integer ? Math.floor(damage) : damage
}
// 减伤计算函数
const getAamageReduction = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 - (data.damageReduction || 0) / 100) // 减伤护盾
  return integer ? Math.floor(damage) : damage
}
// 护甲计算函数
const getEnemyDefence = (damage: number, data: buffObj, integer: boolean = true) => {
  damage = damage * (1 - (data.enemyDefence || 0) / 100) // 抗性
  return integer ? Math.floor(damage) : damage
}
// 属性抵抗计算函数
const getAttributeResistance = (damage: number, data: buffObj, integer: boolean = true) => {
