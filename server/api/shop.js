...
export function updateMatchingItems(arrayA, arrayB, sourceKey, targetKey, key = 'id') {
  const mapA = Object.fromEntries(arrayA.map((item) => [item[key], item]))
          ...
          .map((item) => {
            if (+item.shopId === +shopId) {
              return {
                ...
                maxPrice: Math.floor(
                  (mData.data[item.id].base * mData.data[item.id].maxRate) / 100,
                ), // 最大购买价为 120% 的最终价格
                price: item.price,
                type: item.type,
              }
...
export const buyUpdata = async (req, res) => {
  ...
  const saveData = buyAllList.data.filter(
