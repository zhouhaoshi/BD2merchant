<template>
  <el-card>
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      <el-form-item label="章节">
        <el-select
          ...
          @change="changeMap"
        > 
          <el-option v-for="item in mapList" :class="isCompleted(item)" :key="item.shopId" :value="item.shopId" :label="`${item.label}.${item.value}`" />
        </el-select>
      </el-form-item>
      ...
      <el-form-item label="讨价还价" v-if="formInline.type === 'buy'">
        <el-select
          ...
          placeholder="请选择讨价还价级别"
        > 
          <el-option v-for="item in discountList" :key="item.value" :value="item.value" :label="item.lable" />
        </el-select>
      </el-form-item>
    ...
    <div class="shop_box" v-if="shopList">
      <template v-if="formInline.type === 'buy'">
        <div v-if="formInline.discount === 0 || formInline.discount === 0.1" :class="formInline.discount === 0.1 ? 'normal': 'abnormal'">
          <img style="height: 50px; width: 50px;" src="/src/assets/img/icon.png">
          <div>
            <div>天賦特效藥</div>
            <div><img src="/src/assets/img/icon_resource60005_88.webp">{{ formInline.discount !== 0.1 ? 50 : 45 }}</div>
          </div>
        </div>
        <div 
          ...
          :class="
            item.type === 'cuisine' ? 'cuisine_food' : 
            getPrice(+item.price, formInline.discount) < item.maxPrice ? 'normal': 'abnormal'
          "
        >
          <img :src="item.url">
          <div>
            <div>{{ item.name }} {{ item.maxPrice - getPrice(+item.price, formInline.discount) }}</div>
            <div><img src="/src/assets/img/icon_resource60005_88.webp">{{ getPrice(+item.price, formInline.discount) }}</div>
          </div>
        </div>
      </template>
      <template v-else>
  ...
  <el-card style="margin-top: 20px;" v-if="senceList.length">
    <el-radio-group v-model="sence" size="large" @change="changeSence">
      <el-radio-button v-for="(item, index) in senceList" :value="index" :key="item.url"> {{ item.name }}</el-radio-button>
    </el-radio-group>
    <SenceImage 
      isMove
      :size="512"
      ...
      @move="(moveData) => 
        sence + moveData < 0 ? sence = senceList.length - 1 :
        sence + moveData > senceList.length - 1 ? sence = 0 :
        sence += moveData"
    />
  </el-card>
</template>
