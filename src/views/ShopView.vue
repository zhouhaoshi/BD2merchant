<template>
  <el-card>
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      <el-form-item label="章节">
        <el-select
          ...
          @change="onSubmit"
        > 
          <el-option v-for="item in mapList" :key="item.shopId" :value="item.shopId" :label="`${item.label}.${item.value}`" />
        </el-select>
      </el-form-item>
      <el-form-item>
    ...
    <draggable group='label' item-key="index" class="shop_box" :list='shopList' :disabled='false' :force-fallback='true'>
...
const clickDelete = (data:shopObj, dIndex?: number) => {
  ElMessageBox.alert(`是否删除商品: ${data.name}？`, '删除提示', {
    ...
    callback: (action:string) => {
      if(action === 'confirm') {
        //                                                                        防止id一致，使用下标和id同时标记
        shopList.value = shopList.value!.filter((item: shopObj, index: number) => `${item.id}${index}` !== `${data.id}${dIndex}`)
      }
    },
  })
}
